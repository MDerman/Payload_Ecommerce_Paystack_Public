# Payload E-Commerce - TheNesquikOutlet.co.za:

- Paystack integration (stripe skeleton from below Payload E-Commerce Template docs remains)
- Sendgrid mail sending (configure in node server.ts)
- Docker compose included to run on https on your own VM
- Github Action included to deploy automatically on master branch commits
- use Mongo Atlas for database connection
...
2. npm install --legay-peer-deps, npm dev
