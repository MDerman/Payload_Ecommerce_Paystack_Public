...
export function formatRawPrice(price: number) {
  const returnPrice = (price / 100).toLocaleString('en-ZA', {
    style: 'currency',
    currency: 'zar',
  })
  return returnPrice;
...
export const priceFromJSON = (priceJSON: string, quantity: number = 1, raw?: boolean): string => {
  ...
  if (priceJSON) {
    try {
     ...
     if (parsed.currency && parsed.currency == "zar") {
        price = "R " + (priceValue / 100).toFixed(2)/*.toLocaleString('en-US', {
          style: 'currency',
