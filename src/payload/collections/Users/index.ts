...
const Users: CollectionConfig = {
  ...
  hooks: {
   /* beforeChange: [welcomeAfterCreate],*/
    // afterChange: [loginAfterCreate],
  },
  auth: {
    /*verify: {
      generateEmailSubject: () => 'Verify your email',
      generateEmailHTML: ({user, token}) => GenerateVerificationEmail({user, token}),
    },*/
    forgotPassword: {
      generateEmailSubject: () => 'Reset your password',
      // @ts-ignore
      generateEmailHTML: ({token}) => generateForgotPasswordEmail({token}),
    },
  },
  endpoints: [
    {
